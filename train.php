...
if (isset($_GET['train']))
{
    ...
    for ($i = 1; $i <= $_GET['times'] && $ir['energy'] > 0; $i++)
    {
        ...
        {
            $gain =
                    rand(1, 3) / rand(800, 850) * rand(950, 1000)
                            * (($ir['will'] + 75) / 150);
            $tgain += $gain;
            if ($_GET['train'] == "IQ")
            {
...
You can train up to {$ir['energy']} times.<br /><form action='train.php' method='get'>
...
print "Running out of will power? Want to pay a visit to the library? <font color = 'darkblue'><a href='library.php'>Click here</a></font>";
